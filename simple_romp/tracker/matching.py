...
def euc_dist(atrans, btrans):
    euc_dists = np.zeros((len(atrans), len(btrans)), dtype=float)
    if euc_dists.size == 0:
        return euc_dists
...
def euclidean_distance(atracks, btracks, dim=4):
    """
    ...
    if (len(atracks) > 0 and isinstance(atracks[0], np.ndarray)) or (len(btracks) > 0 and isinstance(btracks[0], np.ndarray)):
        ...
        atrans = [track.trans[:dim] for track in atracks]
        btrans = [track.trans[:dim] for track in btracks]
