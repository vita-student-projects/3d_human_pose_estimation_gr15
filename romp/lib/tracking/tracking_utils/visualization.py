...
def plot_tracking(image, centers, obj_ids, scores=None, frame_id=0, fps=0., ids2=None):
    ...
    cv2.putText(im, 'frame: %d fps: %.2f num: %d' % (frame_id, fps, len(centers)),
    ...
    for i, center in enumerate(centers):
        ...
        cv2.circle(im, center, color=color, radius=16,  thickness=-1)
        cv2.putText(im, id_text, (center[0], center[1] - 30), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255),
...
def plot_detections(image, tlbrs, scores=None, color=(255, 0, 0), ids=None):
    ...
    for i, det in enumerate(tlbrs):
        x1, y1, x2, y2 = np.asarray(det[:4], dtype=int)
        if len(det) >= 7:
